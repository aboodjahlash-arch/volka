# VOLK AI Backend Service

English | [中文](README_zh.md)

VOLK AI is an AI agent system specialized in cybersecurity and ethical hacking, based on FastAPI and LangChain chat models. The backend adopts Domain-Driven Design (DDD) architecture, supporting security reconnaissance, penetration testing, vulnerability assessment, file operations, Shell command execution, browser automation, and integrated [OpenClaw](https://github.com/anthropics/openclaw) AI assistant management (Claw).
...
6. **Claw (VOLK AI)**: Per-user OpenClaw container lifecycle management, chat history merge (MongoDB + OpenClaw `.jsonl` sessions), WebSocket real-time messaging, file upload/resolve, and OpenAI-compatible LLM proxy for Claw containers
...
# Primary LLM: MiniMax-M3 from OpenRouter
...
SANDBOX_IMAGE=volk-ai-sandbox:latest          # Sandbox environment Docker image
SANDBOX_NAME_PREFIX=sandbox              # Sandbox container name prefix
SANDBOX_TTL_MINUTES=30                   # Sandbox container time-to-live (minutes)
SANDBOX_NETWORK=volk-ai-network            # Docker network name for communication between sandbox containers

# Database configuration
MONGODB_URI=mongodb://localhost:27017    # MongoDB connection URL
MONGODB_DATABASE=volk_ai                   # MongoDB database name
...
```bash
# Build Docker image
docker build -t volk-ai-agent .

# Run container
docker run -p 8000:8000 --env-file .env -v /var/run/docker.sock:/var/run/docker.sock volk-ai-agent
```
