...
VOLK AI 是一个专注于网络安全与道德黑客的 AI Agent 系统，基于 FastAPI 和 LangChain Chat Model 构建。该后端采用领域驱动设计(DDD)架构，支持安全侦察、渗透测试、漏洞评估、文件操作、Shell命令执行、浏览器自动化，以及集成 [OpenClaw](https://github.com/anthropics/openclaw) AI 助手管理（Claw）等功能。
...
6. **Claw（VOLK AI）**：为每个用户管理 OpenClaw 容器生命周期，合并聊天历史（MongoDB + OpenClaw `.jsonl` 会话），WebSocket 实时通信，文件上传/解析，以及为 Claw 容器提供 OpenAI 兼容 LLM 代理
...
# 主要 LLM：OpenRouter 上的 MiniMax-M3
# 从此处获取 API 密钥：https://openrouter.ai/keys
# =====================================================================
API_KEY=sk-or-v1-your-openrouter-api-key-here        # OpenRouter API 密钥
API_BASE=https://openrouter.ai/api/v1                 # OpenRouter 基础 URL

# Model configuration
MODEL_NAME=minimax/minimax-m3                        # 使用的模型名称（OpenRouter 上的 MiniMax-M3）
MODEL_PROVIDER=openai                                # LangChain 模型供应商（OpenRouter 兼容 OpenAI API）
TEMPERATURE=0.7                                      # 模型温度参数
MAX_TOKENS=2000                                      # 模型单次请求最大输出 token 数量

# Sandbox configuration
SANDBOX_IMAGE=volk-ai-sandbox:latest          # 沙盒环境 Docker 镜像
SANDBOX_NAME_PREFIX=sandbox              # 沙盒容器名称前缀
SANDBOX_TTL_MINUTES=30                   # 沙盒容器生存时间（分钟）
SANDBOX_NETWORK=volk-ai-network            # Docker 网络名称，用于沙盒容器间通信

# Database configuration
MONGODB_URI=mongodb://localhost:27017    # MongoDB 连接 URL
MONGODB_DATABASE=volk_ai                   # MongoDB 数据库名称
