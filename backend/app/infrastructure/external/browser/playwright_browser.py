...
class PlaywrightBrowser:
    ...
    def __init__(self, cdp_url: str):
        ...
        kwargs = dict(
            ...
            default_headers={
                "HTTP-Referer": "https://volk-ai.local",
                "X-Title": "VOLK AI",
            },
        )
    ...
    async def _extract_interactive_elements(self) -> List[str]:
        ...
        interactive_elements = await self.page.evaluate("""() => {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                // Only add data-volk-ai-id attribute to elements that meet the conditions
                element.setAttribute('data-volk-ai-id', `volk-ai-element-${validElementIndex}`);

                // Build selector - using only data-volk-ai-id
                const selector = `[data-volk-ai-id="volk-ai-element-${validElementIndex}"]`;
    ...
    async def _get_element_by_index(self, index: int) -> Optional[Any]:
        """Get element by index using data-volk-ai-id selector
        ...
        # Use data-volk-ai-id selector
        selector = f'[data-volk-ai-id="volk-ai-element-{index}"]'
