...
@router.get("/credits", response_model=APIResponse[dict])
async def get_credits(
    current_user: User = Depends(get_current_user)
) -> APIResponse[dict]:
    """Get current user's daily credits status"""
    return APIResponse.success({
        "credits_remaining": current_user.credits_remaining(),
        "daily_limit": 1 if not current_user.is_vip() else None,
        "is_vip": current_user.is_vip(),
        "reset_date": current_user.last_credit_reset_date
    })
