...
async def upload_claw_file(
    ...
    """Upload a file from the claw workspace to VOLK AI storage (authenticated by claw API key)"""
...
async def resolve_claw_file_meta(
    ...
    """Get file metadata for volk-ai-file:// resolution (authenticated by claw API key)"""
    user_id = await claw_service.verify_api_key(x_claw_api_key)
    if not user_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid claw API key")
...
async def resolve_claw_file_download(
    ...
    """Download file content for volk-ai-file:// resolution (authenticated by claw API key)"""
    user_id = await claw_service.verify_api_key(x_claw_api_key)
    if not user_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid claw API key")
    try:
...
async def claw_ws(websocket: WebSocket, token: str | None = None):
    ...
    async def _process_files(file_ids: list[str], uid: str) -> tuple[str, list[ClawAttachment]]:
        ...
        (VOLK_AI_FILE reference tags for the message, attachment metadata for history).
        ...
        for fid in file_ids:
            try:
                ...
                if not claw_base_url:
                    refs.append(f'<VOLK_AI_FILE name="{filename}" id="{fid}" status="no_claw" />')
                    continue
                ...
                refs.append(
                    f'<VOLK_AI_FILE path="{local_path}" name="{filename}" '
                ...
                refs.append(
                    f'<VOLK_AI_FILE name="{fid}" id="{fid}" status="download_failed" '
                    f'reason="{str(e)[:100]}" />'
                )
