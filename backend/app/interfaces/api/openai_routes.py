"""
OpenAI-compatible API proxy for volk-ai-claw.
...
async def _stream_llm_response(
    ...
    """Stream LLM response from the configured backend (default: OpenRouter)"""
    api_base = settings.api_base or "https://openrouter.ai/api/v1"
...
async def chat_completions(request: Request):
    ...
    Authenticates using per-user volk-ai API keys and forwards to the configured LLM backend.
    """
    ...
    if settings.model_name and body.get("model") in ("default", "volk-ai-proxy/default", None):
