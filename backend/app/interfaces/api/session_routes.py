...
from app.application.errors.exceptions import NotFoundError, UnauthorizedError, CreditLimitError, BadRequestError
...
async def chat(
    ...
    # Consume credit on first message (task start)
    if not current_user.is_vip():
        user_repo = MongoUserRepository()
        fresh_user = await user_repo.get_user_by_id(current_user.id)
        if fresh_user and not fresh_user.has_credits():
            raise CreditLimitError()
        if fresh_user:
            fresh_user.consume_credit()
            await user_repo.update_user(fresh_user)
