...
echo "[entrypoint] VOLK AI API base URL: ${VOLK_AI_API_BASE_URL:-http://backend:8000}/v1"
...
cat > "${CONFIG_FILE}" << EOF
{
  ...
  "plugins": {
    "load": {
      "paths": [
        "/home/node/.openclaw/extensions/volk-ai-claw"
      ]
    },
    "entries": {
      "volk-ai-claw": {
  ...
  "models": {
    ...
    "providers": {
      "volk-ai-proxy": {
        "baseUrl": "${VOLK_AI_API_BASE_URL:-http://backend:8000}/v1",
        "apiKey": "${VOLK_AI_API_KEY}",
