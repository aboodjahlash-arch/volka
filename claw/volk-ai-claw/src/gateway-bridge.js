 ...
 * When the agent calls volk_ai_upload_file, OpenClaw invokes the tool's execute()
...
export class GatewayBridge {
  ...
  /**
   * Called by the volk_ai_upload_file tool execute() in index.js after a
  ...
  async sendPrompt(sessionId, message, requestId) {
    // Pre-process: resolve volk-ai-file:// URIs → <VOLK_AI_FILE /> tags
    let resolvedMessage = message;
    if (this.fileResolver && typeof message === 'string' && message.includes('volk-ai-file://')) {
      try {
    ...
    const sessionKey = `volk-ai:${sessionId}`;
    ...
    const sent = this.gatewayClient.send({
      ...
      params: {
        ...
        idempotencyKey: `volk-ai_${sessionId}_${Date.now()}`,
      }
    });
