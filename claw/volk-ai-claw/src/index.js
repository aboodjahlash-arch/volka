...
import { VolkAiClawHttpServer } from './http-server.js';
import { VolkAiFileResolver } from './volk-ai-file-resolver.js';

// ─── volk_ai_upload_file tool definition ───────────────────────────────────────

const VOLK_AI_UPLOAD_TOOL_NAME = 'volk_ai_upload_file';

const VOLK_AI_UPLOAD_TOOL_DESCRIPTION =
  'Upload local files from the workspace to the VOLK AI platform so the user can download them. ' +
...
const VOLK_AI_UPLOAD_TOOL_SCHEMA = {
  type: 'object',
  required: ['paths'],
...
async function uploadFileToVolkAi({ filePath, fileName, volkAiApiBaseUrl, volkAiApiKey }) {
  ...
  try {
    const uploadUrl = `${volkAiApiBaseUrl}/api/v1/claw/upload`;
    const response = await fetch(uploadUrl, {
      method: 'POST',
      headers: { 'X-Claw-Api-Key': volkAiApiKey },
      body: formData,
...
function getUploadMetaDir(openclawHome) {
  return path.join(openclawHome || '/home/node/.openclaw', 'plugins', 'volk-ai-claw', UPLOAD_META_DIR_NAME);
}
...
const plugin = {
  id: 'volk-ai-claw',
  name: 'volk-ai-claw',
  description: 'Connector plugin that bridges VOLK AI backend with the local OpenClaw Gateway.',
  ...
  register(context) {
    ...
    const volkAiApiBaseUrl = process.env.VOLK_AI_API_BASE_URL || 'http://backend:8000';
    const volkAiApiKey = process.env.VOLK_AI_API_KEY || '';
    ...
    context.registerTool({
      name: VOLK_AI_UPLOAD_TOOL_NAME,
      description: VOLK_AI_UPLOAD_TOOL_DESCRIPTION,
      parameters: VOLK_AI_UPLOAD_TOOL_SCHEMA,

      async execute(toolCallId, args) {
        const logger = context.logger;
        const rawPaths = args?.paths || [];
        logger?.info?.(`[volk_ai_upload_file] called toolCallId=${toolCallId} paths=${JSON.stringify(rawPaths)}`);
        ...
        for (const { filePath, fileName, size } of validFiles) {
          try {
            const fileInfo = await uploadFileToVolkAi({ filePath, fileName, volkAiApiBaseUrl, volkAiApiKey });
            uploaded.push(fileInfo);
            logger?.info?.(`[volk_ai_upload_file] uploaded ${fileName} -> file_id=${fileInfo.file_id}`);
            ...
            logger?.warn?.(`[volk_ai_upload_file] upload failed for ${fileName}: ${msg}`);
          }
        }
        ...
        logger?.info?.(`[volk_ai_upload_file] result=${JSON.stringify(result)}`);
    ...
    context.registerService({
      ...
      start: (runtime) => {
        ...
        if (cfg.log.enabled) {
          logger?.info?.(`[volk-ai-claw] starting with gateway=${cfg.gateway.url} server=${cfg.server.host}:${cfg.server.port}`);
        }
        ...
        const fileResolver = new VolkAiFileResolver({
          volkAiApiBaseUrl,
          volkAiApiKey,
        ...
        httpServer = new VolkAiClawHttpServer({
