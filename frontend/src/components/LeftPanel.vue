<template>
  <div :class="isLeftPanelShow ?
    ...
    <div
      ...
      <!-- Credits display -->
      <div class="px-3 py-2 border-t border-[var(--border-main)] flex-shrink-0">
        <div class="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm text-[var(--text-secondary)]">
          <Zap :size="14" class="flex-shrink-0" :class="isVip ? 'text-yellow-500' : creditsRemaining > 0 ? 'text-green-500' : 'text-red-500'" />
          <span v-if="isVip" class="text-xs">{{ t('Unlimited') }} ✨</span>
          <span v-else class="text-xs">{{ creditsRemaining }}/1 {{ t('daily tasks') }}</span>
        </div>
      </div>
    </div>
  </div>
...
import { PanelLeft, SquarePen, Command, MessageSquareDashed, ChevronUp, Zap } from 'lucide-vue-next';
...
const creditsRemaining = computed(() => currentUser.value?.credits_remaining ?? 0)
const isVip = computed(() => currentUser.value?.is_vip ?? false)
