      ...
      <div class="w-full max-w-full sm:max-w-[768px] sm:min-w-[390px] mx-auto mt-[180px] mb-auto">
        ...
        <div class="flex flex-col gap-1 w-full">
          ...
          <div class="flex justify-center pt-2">
            <span v-if="currentUser?.is_vip" class="text-xs text-yellow-500">✨ {{ $t('Unlimited tasks') }}</span>
            <span v-else-if="currentUser?.credits_remaining && currentUser.credits_remaining > 0" class="text-xs text-[var(--text-tertiary)]">
              {{ currentUser.credits_remaining }}/1 {{ $t('task available today') }}
            </span>
            <span v-else-if="currentUser" class="text-xs text-red-400">
...
import VolkAiLogoTextIcon from '../components/icons/VolkAiLogoTextIcon.vue';
...
const avatarLetter = computed(() => {
  return currentUser.value?.fullname?.charAt(0)?.toUpperCase() || 'O';
});
...
const handleSubmit = async () => {
  if (message.value.trim() && !isSubmitting.value) {
    ...
    try {
      ...
      if (error?.response?.status === 429 || msg.toLowerCase().includes('limit')) {
        showErrorToast(t('Daily task limit reached. You can run 1 task per day. Try again tomorrow.'));
      } else {
